...
--
-- Host: localhost:3307
-- Generation Time: Oct 01, 2023 at 02:57 PM
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `View_User_Projects_By_ID` (IN `UID` VARCHAR(20))   SELECT * FROM `view_user_project` where UID=UID$$
...
('LDVYV9CN0TS564TEAXJM', 'Farming Simulation,Game'),
...
INSERT INTO `project_meta` (`PID`, `MID`, `Download`, `Repolink`, `LiveLink`, `Screenshot`, `Description`) VALUES
('2Q7IOKHHPSIVNCJBRK73', 'A7ZXF5663PV19YQHZ7QT', 'https://github.com/jay/fs', 'https://github.com/jay/fs', '', 3, '<strong>Harvest Haven</strong> is your very own virtual farming paradise. Immerse yourself in the serenity of the countryside as you cultivate crops, raise animals, and create the farm of your dreams. Engage with a charming community, participate in seasonal events, and customize your farm to reflect your unique vision. Whether youre a seasoned farmer or a newcomer to the genre, Harvest Haven offers a relaxing and rewarding gaming experience where hard work and creativity are the keys to success.\n<br><br>\nVenture beyond your farm to explore the picturesque landscape, uncover hidden treasures, and forge connections with both nature and your fellow players in multiplayer mode. Harvest Haven is not just a game; its an escape to a world where the simple joys of farming come to life, providing hours of wholesome entertainment and a sense of accomplishment as you watch your farm flourish and thrive. Dive into Harvest Haven today and experience the joy of rural life in this enchanting farming simulation.\n'),
('NLODH4UJVKVCW84NKM5N', 'DFSC49PH0A6R0FVT4Y28', 'https://github.com/Jay/FreeFIre', 'https://github.com/Jay/FreeFIre', '', 4, 'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed                   do eiusmod tempor incididunt ut labore et dolore magna aliqua.                   Ut enim ad minim veniam, quis nostrud exercitation ullamco                   laboris nisi ut aliquip ex ea commodo consequat. Duis aute                   irure dolor in reprehenderit in voluptate velit esse cillum                   dolore eu fugiat nulla pariatur. Excepteur sint occaecat                   cupidatat non proident, sunt in culpa qui officia deserunt                   mollit anim id est laborum.                   <br />                   <br />                   Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed                   do eiusmod tempor incididunt ut labore et dolore magna aliqua.                   Ut enim ad minim veniam, quis nostrud exercitation ullamco                   laboris nisi ut aliquip ex ea commodo consequat. Duis aute                   irure dolor in reprehenderit in voluptate veli'),
('WZXF3VT50BCG0DMN6NG0', 'LN901X83GQ0NU2J75WPM', 'https://github.com/jay/coe', 'https://github.com/jay/coe', '', 2, 'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed                   do eiusmod tempor incididunt ut labore et dolore magna aliqua.                   Ut enim ad minim veniam, quis nostrud exercitation ullamco                   laboris nisi ut aliquip ex ea commodo consequat. Duis aute                   irure dolor in reprehenderit in voluptate velit esse cillum                   dolore eu fugiat nulla pariatur. Excepteur sint occaecat                   cupidatat non proident, sunt in culpa qui officia deserunt                   mollit anim id est laborum.                   <br />                   <br />                   Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed                   do eiusmod tempor incididunt ut labore et dolore magna aliqua.                   Ut enim ad minim veniam, quis nostrud exercitation ullamco                   laboris nisi ut aliquip ex ea commodo consequat. Duis aute                   irure dolor in reprehenderit in voluptate veli');
...
('hhPKDnDzzHIV6IX4D6Cx', 'abc', 'dsfdf'),
...
('hhPKDnDzzHIV6IX4D6Cx', '22mapbs019@ddu.ac.in', '$2y$10$RjHJ0f7g94yxLdQ/N4gnyOHwNYGPAN8sa4bc9iMmJSzFu/36rFl8u', 1, NULL),
...
('hhPKDnDzzHIV6IX4D6Cx', 'https://www.linkedin.com/in/makwana-bharat/', 'https://www.linkedin.com/in/makwana-bharat/', ''),
...
--
-- Stand-in structure for view `view_user_project`
-- (See below for the actual view)
--
CREATE TABLE `view_user_project` (
`PID` varchar(20)
,`ProjectName` varchar(50)
,`IsVisible` tinyint(1)
,`Rating` float
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `view_project_data`  AS SELECT `u`.`UID` AS `UID`, `p`.`PID` AS `PID`, `mv`.`MID` AS `MID`, `u`.`Username` AS `UserName`, `p`.`NAME` AS `ProjectName`, `mv`.`Views` AS `Views`, `p`.`isVisible` AS `IsVisible`, `p`.`rating` AS `Rating`, `pt`.`TID` AS `TID`, `pm`.`Download` AS `DownloadLink`, `pm`.`LiveLink` AS `LiveLink`, `pm`.`Repolink` AS `Repolink`, `pm`.`Description` AS `Description`, `pm`.`Screenshot` AS `Screenshot`, `c`.`Name` AS `Tag` FROM (((((`mostviewed` `mv` join `project` `p` on(`mv`.`PID` = `p`.`PID`)) join `user` `u` on(`p`.`UID` = `u`.`UID`)) join `project_tag` `pt` on(`p`.`PID` = `pt`.`PID`)) join `category` `c` on(`pt`.`CID` = `c`.`CID`)) join `project_meta` `pm` on(`pm`.`PID` = `p`.`PID`)) ;

-- --------------------------------------------------------

--
-- Structure for view `view_user_project`
--
DROP TABLE IF EXISTS `view_user_project`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `view_user_project`  AS SELECT `p`.`PID` AS `PID`, `p`.`NAME` AS `ProjectName`, `p`.`isVisible` AS `IsVisible`, `p`.`rating` AS `Rating`, `pt`.`TID` AS `TID`, `pm`.`Download` AS `DownloadLink`, `pm`.`LiveLink` AS `LiveLink`, `pm`.`Repolink` AS `Repolink`, `pm`.`Description` AS `Description`, `pm`.`Screenshot` AS `Screenshot`, `c`.`Name` AS `Tag` FROM ((((`project` `p` join `user` `u` on(`p`.`UID` = `u`.`UID`)) join `project_tag` `pt` on(`p`.`PID` = `pt`.`PID`)) join `category` `c` on(`pt`.`CID` = `c`.`CID`)) join `project_meta` `pm` on(`pm`.`PID` = `p`.`PID`)) ;
