...
const ChangePass = (email) => {
        Swal.fire({
          ...
          <div class="w-full h-full flex justify-center items-center flex-col" id="personalInfo">
            ...
            <form class="space-y-4 md:space-y-6 w-96" action="#" onsubmit="resetPassword(event,'${email}')" >
              <div>
                <label for="email" class="block mb-2 text-sm font-medium text-neutral-900 dark:text-white">Email</label>
                <input type="email" name="email" id="email" value="${email}" class="bg-neutral-50 border border-neutral-300 text-neutral-900 sm:text-sm rounded-lg focus:ring-blue-600 focus:border-blue-600 block w-full p-2.5 dark:bg-neutral-700 dark:border-neutral-600 dark:placeholder-neutral-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="name@company.com" required="" desabaled>
              </div>
              <div>
