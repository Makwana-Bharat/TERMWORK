...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($requestData !== null) {
        ...
        $sqlFetchOTP = "SELECT `OTP` FROM `user_credentials` WHERE `UID` = :uid";
        ...
        if ($fetchedOTP === $otp) {
            ...
            $sqlUpdate = "UPDATE `user_credentials` SET `isVerify` = 1, `OTP` = NULL WHERE `UID` = :uid";
            $stmtUpdate = $pdo->prepare($sqlUpdate);
            $stmtUpdate->bindParam(':uid', $uid, PDO::PARAM_STR);
